...
- **Entity locks**: server-enforced exclusive claims, with a FIFO waiter queue — releasing a lock hands it straight to the longest-waiting connection (`EntityLockGranted` push) rather than freeing it for a re-race. `UnlockEntity` flushes the entity's pending dirty fields before sending the release, which is what guarantees the next holder observes the previous holder's edits (guaranteed fields only — `SetUnguaranteed` writes leave the ordered path); this covers hand-rolled `TryLock`/edit/`Unlock` as well as the scoped form. `entity.RunExclusive(body, onComplete, timeout)` is the scoped form: it queues for the lock, runs the body, and releases on every exit path including exceptions. Note that queued waiters are served ahead of `TryLock` pollers, and a handoff raises `OnLocked` (not `OnUnlocked`) on other listeners, so retry loops built on `OnUnlocked` starve. Client code in `Client/Connection/ClientEntityExclusive.cs`; server queue in `GameStateEntity.Lock`/`Unlock`.
- **Conditional actions**: `CreateObject` / `DeleteEntity` (and `entity.Delete`, `manager.CreateObject<T>`) take an optional trailing database action, sent in the same message and run only if the entity operation succeeded for this client. Updates carry them too: `entity.AddReplicatedAction(action)` parks one on the entity (`ClientEntityReplicatedActions.cs`) until its next update is built — per-frame sweep, `UpdateExclusive(onComplete, action)`, or the flush before `Unlock` — forcing a reliable send, even an empty one; several pending travel as one `CompoundDatabaseAction` fanned back out via `ApplyResult`; the condition is "server applied the update"; an entity unregistered with actions pending fails them locally. This is the world ↔ inventory move: in a pickup race the live thread arbitrates, so only the winner's inventory insert runs. The server runs the entity op on the live thread, then queues `ConditionalDatabaseAction` (`GameStateServerActions.cs`), which runs the conditional on the DB thread and hops back to live via `QueueDBReply` to report it. That hop is what guarantees the conditional's reply arrives after the parent's. A skipped conditional replies `ActionConditionNotMet`. The conditional's reply rides a second correlation id (`ConditionalMessageId`, assigned in `RemoteGameConnection.SendMessage`; local connections use a non-zero marker). Allowed conditionals are opt-in via `CanRunConditionally()` (document actions + `CompoundDatabaseAction`). Not a transaction. A later *plain* DB action from the same client can commit before an earlier conditional (plain goes straight to the DB queue; the conditional waits for the live thread) — the guide's "Ordering against your other writes" says to use per-item docs or field-level merges (`unsetKeys` removes a top-level field) and hold same-key writes until the conditional's callback. Exactly-once callbacks: a method that returns always resolves its conditional (no-connection entity methods fire both callbacks synchronously; a disposed `RemoteGameConnection` fails both with `ClientConnectionBrokenError`); a method that *throws* never took it. Public `GameStateActionBase.SkipUnsent(reason)` / `FailLocally(error)` resolve an unsent action. Guide: `docs/guides/DistributedEntities.md` §11; tests: `Assets/Tests/Shared/ConditionalActionTests.cs` + the `TransportSuite` `Conditional*`/`ReplicatedActions*` cases (`--filter "Category=Conditional"`).
...
- Every `IDistributableValueSerializer<T>` declares `ValueSemantics` (`Immutable` / `Mutable`) alongside `ValueType`, and distributed field types put **no constraint on `T`** (the former `where T : IEquatable<T>` is gone — it excluded `ArraySegment<byte>`, which has an `Equals` overload but never declared the interface). `DistributedFieldPolicy.IsUnchanged` in `Client/Connection/DistributedFields.cs` is the single place a `Set` is decided to be a no-op: `Mutable` skips the comparison entirely so every `Set` marks the field dirty; `Immutable` compares via `EqualityComparer<T>.Default`. `Mutable` is for anything whose contents a caller can change without constructing a new value — `BsonSerializer<T>`, `BsonSmallSerializer<T>`, `BlobSerializer` — because `Get()` returns the field's own state, so a value modified in place and passed back to `Set` compares equal to itself and would otherwise be silently dropped. **When writing a serializer, `Mutable` is the safe answer**; declaring `Immutable` asserts the type has meaningful value equality, which the compiler no longer checks. Both properties are compile-time constants on a concrete serializer struct, so the branches fold away in specialized generics. The user-facing rule is *treat `Get()`'s result as read-only; copy, mutate the copy, `Set` the copy* — see the mutable-values section of `docs/guides/DistributedEntities.md`. Coverage: `Assets/Tests/Shared/MutableValueDirtyTests.cs` (`--filter "Category=MutableValue"`).
...
**Suites:** `CoreUnitTests` (serializers/util), `BsonSerializationTests` (persisted-field BSON path), `MutableValueDirtyTests` (`ValueSemantics` dirty detection), `IntegrationTests` (DB CRUD, channels, entities, broadcasts, locks, collections, unsubscribe, exclusive updates, delete-on-disconnect), `ConditionalActionTests` (conditional DB action on create/delete/update: pickup race, chest via `UpdateExclusive` accepted/stale, potion on a client-authoritative entity, skip paths, several pending, reply ordering, persistence, migration-gate rejection), `ExclusiveScopeTests` (`RunExclusive`: handoff guarantee, FIFO ordering, timeout, exception safety, same-client serialization, release on disconnect), `MigrationTests`, `ChannelCleanupTests` (idle reaper, `[Category("Slow")]`), `ChannelListingTests`, and `TransportSuite` — an abstract transport-agnostic battery with four legs: `_Local`, `_EmbeddedTcp` (shared, run in Unity too), `_StandaloneTcp`, `_StandaloneWs` (dotnet-only, in `ImpunityTests/Host/` — launch the real standalone binary out-of-proc via `StandaloneServerFixture` with a generated `config.json`, `/info` readiness probe, and kill-tree teardown).
